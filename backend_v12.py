...
- Searching for Properties: use any combination of city, state, type, and address
  ...
  - python backend_v12.py --username "user" --password "password" --operation search --custom_id "CAL-IRVI-14631DeerParkSt"
...
- Updating a Property: need to provide its custom ID and the updates in a field=value format, separate by space
  - python backend_v12.py --username "user" --password "password" --operation update --custom_id "CAL-IRVI-14631DeerParkSt" --updates "bedrooms=4" "bathrooms=2.5" "price=675000" 

- Deleting a Property: provide its custom ID
  - python backend_v12.py --username "user" --password "password" --operation delete --custom_id "CAL-IRVI-14631DeerParkSt"
...
def update_property(custom_id, updates, username):
    ...
    for db_name in DATABASE_NAMES:
        ...
        if result.matched_count > 0:
            logging.info(GREEN + f"Property with custom_id {custom_id} updated in {db_name}.\n" + RESET)
